...
class SportsList extends StatelessWidget {
  const SportsList({super.key, required this.cubit});
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        CategoryWidget(
          remainingCount: cubit.sportsTasksList.where((e) => !e.isCompleted!).length,
          categryName: 'Sports',
        ),
        ListView.separated(
          shrinkWrap: true,
          physics: const NeverScrollableScrollPhysics(),
          itemCount: cubit.sportsTasksList.length,
          itemBuilder: (context, index) {
            final task = cubit.sportsTasksList[index];
            return TaskItemCard(
              task: task,
              onTap: () {
